...
const Index = () => {
  ...
  const processedData = useMemo(() => {
    ...
    const sectorData = filtered.reduce((acc, item) => {
      ...
      acc[sector].count += 1;
      acc[sector].totalIntensity += item.intensity;
      acc[sector].avgIntensity = acc[sector].totalIntensity / acc[sector].count;
      return acc;
    ...
    const regionData = filtered.reduce((acc, item) => {
      ...
      acc[region].count += 1;
      acc[region].totalIntensity += item.intensity;
      acc[region].avgIntensity = acc[region].totalIntensity / acc[region].count;
      return acc;
    }, {} as Record<string, any>);

    // PESTLE analysis
    const pestleData = filtered.reduce((acc, item) => {
      const pestle = item.pestle || "Unknown";
      if (!acc[pestle]) {
        acc[pestle] = { pestle, count: 0, avgLikelihood: 0, totalLikelihood: 0 };
      }
      acc[pestle].count += 1;
      acc[pestle].totalLikelihood += item.likelihood;
      acc[pestle].avgLikelihood = acc[pestle].totalLikelihood / acc[pestle].count;
      return acc;
    }, {} as Record<string, any>);
    ...
    return {
      ...
      topics: Object.values(topicData).slice(0, 8), // Top 8 topics
      regions: Object.values(regionData),
      pestle: Object.values(pestleData),
      scatter: scatterData,
          ...
          <TabsContent value="analysis" className="space-y-6">
            <Card>
              <CardHeader>
                ...
                <CardDescription>Political, Economic, Social, Technological, Legal, Environmental factors</CardDescription>
              </CardHeader>
              <CardContent>
                <ChartContainer config={chartConfig} className="h-[400px]">
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={processedData.pestle} layout="horizontal">
                      <CartesianGrid strokeDasharray="3 3" />
                      <XAxis type="number" />
                      <YAxis dataKey="pestle" type="category" width={100} />
                      <ChartTooltip content={<ChartTooltipContent />} />
                      <Bar dataKey="avgLikelihood" fill="hsl(var(--chart-3))" name="Avg Likelihood" />
                    </BarChart>
                  </ResponsiveContainer>
                </ChartContainer>
              </CardContent>
